...
openai.api_base = "https://aashopenai.openai.azure.com/"
...
def generate_gpt_chat(prompt,model='gpt-35-turbo',max_tokens=4000,temperature=0.5, agent_mode=False):
    context_for_yoni= f"Answer as a support agent: {prompt}"
    print(f"prompt: {str(context_for_yoni)}, temperature={temperature}, max_tokens={max_tokens}")
    if not agent_mode:
        print("free chat mode")
        messages = [
            {"role":"system","content":"You are an Azure customer support agent whose primary goal is to help users with issues they are experiencing while using azure. You are friendly and concise. You only provide factual answers to queries, and do not provide answers that are not related to Azure."},
        ...
        print("FAQ mode")
        answers = knowledge_management.search_docs(embeddings_utils, prompt)
        answers_string = ""
        ...
        messages = [ 
            {
                "role":"system","content":"""You are an agent that returns answer based on the knowledge base. you will get the answers relevant to the questions, and you will return the answer that is most relevant to the question. 
                You are an Azure customer support agent whose primary goal is to help users with issues they are experiencing while using azure. You are friendly and concise. You only provide factual answers to queries, and do not provide answers that are not related to Azure.
...
def chat(model, model_embedding):
    ...
    # analysis_conf = st.checkbox('analysis configuration')
    if prompt_conf:
        ...
        if st.button('User Bot', key='Continue'):
